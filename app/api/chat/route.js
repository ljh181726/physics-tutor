...
const SVG_FIX_COMMAND = SVG_FIX_INSTRUCTION + `
⚠️【最高等級輸出指令 - 絕對必須執行】：
你剛才生成的所有 SVG 程式碼，必須、絕對、只能包裝在 Markdown 的 xml 程式碼區塊中！也就是開頭必須是 \`\`\`xml ，結尾必須是 \`\`\` 。
嚴禁、嚴禁、嚴禁將 <svg> 標籤直接裸露在一般文字中！這是為了防止學生手機端顯示錯誤！`;
...
4. SVG 繪圖：解答涉及受力分析、光路折射圖或運動軌跡時，必須繪製符合上述物理幾何精確度且適合手機螢幕範圍的 SVG。` + SVG_FIX_COMMAND,
...
3. LaTeX 渲染防錯：
   ...
   - 獨立公式必須使用 $$...$$，且「$$ 符號的上方與下方都必須強制空一行」，格式必須為：
     ...
     切勿與文字同行。
4. SVG 繪圖：涉及幾何圖形、函數圖形、三角函數或向量時，必須繪製 SVG。圖形中的交點、對稱軸、漸近線以及幾何比例（如等腰、相似、全等）在視覺上必須展現出嚴格的數學正確性，且整體佈局必須縮小在手機螢幕安全範圍內。` + SVG_FIX_COMMAND,
...
4. SVG 繪圖：涉及到實驗裝置圖、分子結構、溶液混合示意圖或能階圖時，必須繪製 SVG。分子結構中的鍵角必須符合實際化學幾何角度；能階圖的線段高度差必須正比於能量差（$\\Delta E$）。所有元素需緊湊排列以適應手機螢幕。` + SVG_FIX_COMMAND,
...
4. SVG 繪圖：涉及到細胞構造、生理作用流程圖、孟德爾遺傳棋盤方格、生態系能量金字塔時，必須繪製 SVG。能量金字塔的各層矩形面積必須嚴格符合能量傳遞的十等律（逐層縮小約九成比例）；所有圖表與流程必須在 420x350 畫布內緊湊呈現，防止手機端溢出。` + SVG_FIX_COMMAND,
...
4. SVG 繪圖：涉及到天球座標、太陽日與恆星日、板塊邊界、洋流氣壓系統時，必須繪製 SVG。天球模型中的黃道與天球赤道夾角在視覺比例上必須合理；地層倒轉或斷層錯動的方向必須與文字敘述的力學機制完全吻合，且圖形需配合手機螢幕做緊湊化佈局。` + SVG_FIX_COMMAND
};
